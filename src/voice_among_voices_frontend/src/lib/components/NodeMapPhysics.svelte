<script lang="ts">
        ...
        function applyMagnetismForces() {
          physicsBodies.forEach((body, i) => {
            ...
            bodiesWithinReach.forEach((bodyWithinReach) => {
              ...
              const bodyWithinReachPos = bodyWithinReach.translation();
      ...
      if (elapsed > interval) {
        ...
        if (context) {
          ...
          if (lastAppliedRotation !== mapRotation.current) {
            ...
            // Apply new rotation
            context.rotate(
              ((lastAppliedRotation - mapRotation.current) / 180) * Math.PI +
                Math.PI,
            );
  ...
  async function handleDrop(e: DragEvent) {
    ...
    const nodeRadius = parseFloat(e.dataTransfer?.getData("nodeRadius") || "5");
  ...
  function isNodeTouchedByPlayhead(
    ...
    const rotatedNodeX =
      nodeX * Math.cos(rotationRadians) + nodeY * Math.sin(rotationRadians);
    const rotatedNodeY =
      nodeX * Math.sin(rotationRadians) - nodeY * Math.cos(rotationRadians);
    ...
    if (playHeadAngle % 180 === 0) {
      ...
      distanceToTangent =
        Math.abs(aTangent * rotatedNodeX + bTangent * rotatedNodeY + cTangent) /
